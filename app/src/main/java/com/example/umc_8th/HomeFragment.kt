...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 자동 슬라이드 효과 추가 (핸들러 이용)
        val adhandler = Handler(Looper.getMainLooper())
        val runnable = object : Runnable {
            override fun run() {
                val currentItem = binding.adBannerVp.currentItem
                val nextItem = (currentItem + 1) % adbannerlist.size
                binding.adBannerVp.setCurrentItem(nextItem, true)
                adhandler.postDelayed(this, 3000) // 3초마다 변경
            }
        }
        ...
        val toprunnable = object : Runnable {
            override fun run() {
                val currentItem = binding.topBannerVp.currentItem
                val nextItem = (currentItem + 1) % topbannerlist.size
